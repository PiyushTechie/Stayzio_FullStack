...
const index = async (req, res) => {
  res.render("listings/index", { showSearch: true, csrfToken: req.csrfToken() });
};

const apiIndex = async (req, res) => {
  try {
    const { q, page = 1, minPrice, maxPrice, beds, propertyType, amenities, category } = req.query;
    const limit = 12;
    ...
    if (beds) {
      findQuery.beds = { $gte: Number(beds) };
    }

    if (propertyType) {
      findQuery.propertyType = propertyType;
    }

    if (amenities) {
      const amenitiesArr = amenities.split(',').map(a => a.trim());
      findQuery.amenities = { $all: amenitiesArr };
    }
...
const renderNewForm = (req, res) => {
  res.render("listings/new", { csrfToken: req.csrfToken() });
};
...
const renderEditForm = async (req, res) => {
  ...
  res.render("listings/edit", { listing, csrfToken: req.csrfToken() });
};
